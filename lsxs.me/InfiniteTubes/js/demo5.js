...
Tunnel.prototype.updateCurve = function(delta) {
  ...
  var f, p, h, rgb;
  let arr = [1, 2, 3];
    // let shuffle1 = arr[Math.floor(Math.random() * arr.length)];
    // let shuffle2 = arr[Math.floor(Math.random() * arr.length)];
    // let shuffle3 = arr[Math.floor(Math.random() * arr.length)];
    let coin = arr[Math.floor(Math.random() * arr.length)];
  for (var i = 0; i < this.tubeGeometry.faces.length; i++) {
    f = this.tubeGeometry.faces[i];
    p = this.tubeGeometry.vertices[f.a];
    h = (Math.floor(Math.abs(noise.simplex3(p.x * 2, p.y * 4, p.z * 2 + delta)) * 80 * 100) * 0.01 + 180) / 360;
    rgb = hslToRgb(h, 1,0.6)
    color = new THREE.Color(rgb[1], rgb[0], rgb[2]);
    f.color = color;
  }
